...
$img = ['image/gif','image/bmp','image/jpg','image/jpeg'];
if(!in_array($_FILES['files']['type'],$img)){
	echo json_encode(['status'=>0,'message'=>'您上传的图片类型不支持','url'=>$_FILES['files']['type']]);exit;
}
if($_FILES['files']['size']>= 5*1024*1024){
	echo json_encode(['status'=>0,'message'=>'您上传的图片过大','url'=>""]);exit;
}
